%%
adc_data20240519_20mm_perp_170_radar_disp = []; % 1*56
adc_data20240519_20mm_perp_170_radar_freq = [];
distance = 1900;
...
for wii = 1:7
    clearvars -except wii adc_data20240519_20mm_perp_170_radar_freq adc_data20240519_20mm_perp_170_radar_disp distance r1_0_all r2_0_all theta_all;
                freq_0 = 77e9; 
                lambda = 3.89e-3;
                Fs = 128/0.024/3; % periodicity = 24 ，slow sample rate 1777
                cut_range_bin = 20; % number of range bins shown in the result
                low_cut_range_bin = 5; %5
                Chirp_fs = 1e7; 
                numADCSamples = 256;
                c = 3e8; Slope = 50.018e12;
                chirp_fft_resol = Chirp_fs / numADCSamples;

                filename = sprintf('adc_data_20240603_20mm_260cm_perp_50_%d.bin',wii);   
%                 filename = sprintf('adc_data_20240519_20mm_perp_170_5.bin');   
    ...
    for ni = 1:8
        close all;
        clearvars -except wii ni T1_Rx_FFT_all T2_Rx_FFT_all T3_Rx_FFT_all T1_Rx_chirp_all T2_Rx_chirp_all T3_Rx_chirp_all adc_data20240519_20mm_perp_170_radar_freq adc_data20240519_20mm_perp_170_radar_disp distance r1_0_all r2_0_all theta_all;
...
%     grid on;
    [~,angle_r2] = max(Pmusic);
    angle_r2 = -90 + mr2d*(angle_r2-1);
    disp('end MUSIC...');

    theta1 = angle_r2 - angle_r1;
%     theta1 = 49;
    theta_all = [theta_all,theta1];
    ...
    Y_single_side = Y_single_side / N;  %由于我们仅保留了单侧频谱，需要调整幅度 
%     % figure;
...
%     figure;
    ...
    %% czt
    r1_0_vector = []; r2_0_vector = [];b_vector =[];
    for c_point = 1:length(T1_Rx_chirp)
        x = T1_Rx_chirp(c_point,:,1); % 2730 * 256
        M = 256;
...
%         figure;
%         plot(range_czt,abs(x_czt));title('czt');
        abs_czt = abs(x_czt);
        ...
        r1_0_vector = [r1_0_vector,two_ranges(1)];
        r2_0_vector = [r2_0_vector,two_ranges(2)];
    end;
    r1_0 = mean(r1_0_vector);
    r2_0 = mean(r2_0_vector);
    r1_0_all = [r1_0_all,r1_0];
    r2_0_all = [r2_0_all,r2_0];
...
%                     title("FFT on r2");grid on;
        ...
        f1_r1_r1 = diff(f1,r1,2);
        f1_r1_r2 = diff(diff(f1,r1),r2);
        f1_r2_r1 = diff(diff(f1,r2),r1);
        f1_r2_r2 = diff(f1,r2,2);

        f2_r1_r1 = diff(f2,r1,2);
        f2_r1_r2 = diff(diff(f2,r1),r2);
        f2_r2_r1 = diff(diff(f2,r2),r1);
        f2_r2_r2 = diff(f2,r2,2);
...
%         adc_data20240519_20mm_para_170_radar_disp = [adc_data20240519_20mm_para_170_radar_disp,ptp_avg*1000]; %mm
...
%         title("vibration of perpendicular axis");
        ...
        adc_data20240519_20mm_perp_170_radar_disp = [adc_data20240519_20mm_perp_170_radar_disp,ptp_avg*1000]; %mm
...
%         adc_data20240519_20mm_para_70_radar_freq = [adc_data20240519_20mm_para_70_radar_freq,correspondingFrequency];
%         figure;
%         plot(frequencies(1:chirps_num/2+1),abs(Y_single_side));
%         title("FFT on parallel");
        ...
        [maxY, indexOfMax] = max(Y_single_side);  
        correspondingFrequency = frequencies(indexOfMax);
%         adc_data20240519_20mm_perp_170_radar_freq = [adc_data20240519_20mm_perp_170_radar_freq,correspondingFrequency];
%         figure;
%         plot(frequencies(1:chirps_num/2+1),abs(Y_single_side));
%         title("FFT on perpendicular");
        ...
        czt_result = czt(delta_f2_double,M,W,A);
        ff = startf:intervalf:startf+intervalf*(M-1);
        figure;plot(ff,abs(czt_result));
        [maxY, indexOfMax] = max(czt_result);  
        corf = ff(indexOfMax);
        adc_data20240519_20mm_perp_170_radar_freq = [adc_data20240519_20mm_perp_170_radar_freq,corf];
        title('czt');
...
%%
% save('czt_0603_20mm_perp_50_radar_freq_260cm.mat','adc_data20240519_20mm_perp_170_radar_freq');

%%
figure;plot(adc_data20240519_20mm_perp_170_radar_freq,'.');
